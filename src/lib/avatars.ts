// TS port of Aniraku's src/lib/avatars.js — same preset catalog, same URL logic.
...
export function avatarUrl(pathOrUrl?: string | null): string | null {
...
function toAvatar(file: { name: string }, index: number): Avatar {
  return {
    id: index,
    name: file.name,
...
export const AVATAR_LIST: Avatar[] = FILES
  .filter(name => !HIDDEN_AVATARS.has(name))
  .map((name, index) => toAvatar({ name }, index));

const IMAGE_FILE_PATTERN = /\.(?:avif|gif|jpe?g|png|webp)$/i;

async function listStorageFiles(prefix = '', visited = new Set<string>()): Promise<Array<{ name: string; [key: string]: unknown }>> {
  ...
  const files: Array<{ name: string; [key: string]: unknown }> = [];
...
export function defaultAvatar(seed: number | string = 0, avatars: Avatar[] = AVATAR_LIST): Avatar {
