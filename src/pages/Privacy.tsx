// TS port of Aniraku's src/pages/Privacy.jsx — content kept verbatim in the
...
const Privacy = () => {
  ...
  useSeo({
    title: 'Privacy Policy | Aniraku',
    description:
      'Privacy policy for Aniraku — what information the client may handle, why it is needed, which parts stay in your browser, and the choices you have.',
  ...
  return (
    ...
    >
      ...
      <ul>
        ...
        <li><strong>Network and request data:</strong> when your browser requests AniList metadata directly, AniList may receive your IP address, browser/network information, and the GraphQL query needed to answer the request. Aniraku does not route those AniList metadata requests through a Aniraku server.</li>
      </ul>
      ...
      <p>Browser privacy controls, content blockers, or disabled storage can prevent some features from working. Optional Vercel Analytics or Speed Insights scripts, when enabled on a deployment, may process technical usage information under Vercel’s policies. GIF reactions may involve GIPHY or another configured GIF provider. You can block these requests through browser or network controls, although some optional features may then be unavailable.</p>

      <h2 id='retention'>6. Retention, deletion, and children</h2>
      <p>We retain account, library, and community records for as long as needed to provide the feature, keep the service secure, resolve disputes, comply with law, or maintain legitimate operational records. Browser-only bookmarks, watch history, episode progress, player preferences, NSFW preference, reaction cache, and support-prompt state remain on the device until cleared or removed by the relevant feature. Clearing browser storage does not delete cloud data, and deleting an account does not necessarily erase copies already held by third-party providers or lawful backups.</p>
