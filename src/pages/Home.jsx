  ...
  return (
    <ListStack>
      {items.slice(0, 6).map((item) => (
        <ListItem key={item.id} to={detailHref(item)} title={`Open ${titleFor(item)}`}>
          <img src={posterFor(item)} alt="" loading="lazy" />
          <div>
            <h3>{titleFor(item)}</h3>
...
function Home() {
  ...
  const finishedList = useMemo(() => [...tvList, ...trendingList]
    .filter((item, index, list) => item?.status === 'FINISHED' && item?.id && list.findIndex((candidate) => candidate.id === item.id) === index)
  ...
  const heroItems = useMemo(() => [...trendingList, ...airingList]
    .filter((item, index, list) => item?.id && list.findIndex((candidate) => candidate.id === item.id) === index)
    .slice(0, 8), [trendingList, airingList])
  const featured = heroItems[featuredIndex] || null
  const upcomingList = useMemo(() => schedule
    .filter((item, index, list) => item?.id && item.id !== featured?.id && list.findIndex((candidate) => candidate.id === item.id) === index)
    .slice(0, 6), [schedule, featured])
  const freshAiring = useMemo(() => airingList.filter((item) => item.id !== featured?.id), [airingList, featured])
  const popularItems = useMemo(() => [...trendingList, ...moviesList]
    .filter((item, index, list) => item?.id && item.id !== featured?.id && list.findIndex((candidate) => candidate.id === item.id) === index), [trendingList, moviesList, featured])
  const topItems = useMemo(() => tvList.filter((item) => item.id !== featured?.id), [tvList, featured])
