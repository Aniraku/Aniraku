...
export const TIMELINE_MARKER_DEFINITIONS = Object.freeze({
  ...
  // Main episode is the derived middle span (no skip window of its own) — a
  // quiet slate band so it reads as "the show" between the two yellow skip
  // targets rather than as a third skip zone.
  main: Object.freeze({ label: 'Main episode', color: 'rgba(148, 163, 184, 0.22)' }),
...
export function getTimelineMarkers(segments = {}, duration = 0) {
  ...
  const normalize = (window) => {
    ...
    const start = Math.max(0, finite(window.start, -1))
    const end = Math.min(total, finite(window.end, -1))
    if (start < 0 || end <= start) return null
    return { start, end }
  }
  const intro = normalize(segments?.intro)
  const outro = normalize(segments?.outro)
  // Chapters are DERIVED from the skip windows, never fetched: no intro →
  // Main starts at 0; no outro → Main runs to the duration; neither intro
  // nor outro → NO markers at all (a lone full-width "Main episode" band
  // would be noise).
  if (!intro && !outro) return []

  const spans = []
  if (intro) spans.push(['intro', intro.start, intro.end])
  const mainStart = intro ? intro.end : 0
  const mainEnd = outro ? outro.start : total
  if (mainEnd > mainStart) spans.push(['main', mainStart, mainEnd])
  if (outro) spans.push(['outro', outro.start, outro.end])

  return spans.map(([type, start, end]) => ({
    type,
 ...
 * verified marked range (intro │ main episode │ outro — see
 * getTimelineMarkers, which derives the middle span from the skip windows).
 */
