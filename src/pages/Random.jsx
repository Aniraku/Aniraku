    ...
    <Page>
      ...
      <Container>
        ...
        {anime && !loading && (
          <>
            <Card>
              <Link to={`/anime/${generateSlug(anime.title?.english || anime.title?.romaji || '')}-${anime.id}`}>
              ...
              <Info>
                ...
                <Actions>
                  <WatchBtn to={`/watch/${generateSlug(anime.title?.english || anime.title?.romaji || '')}-${anime.id}-episode-1`}><FaPlay size={14} /> Watch Now</WatchBtn>
            ...
            {anime.genres && anime.genres.length > 0 && (
              <div style={{ display: 'flex', gap: 6, justifyContent: 'center', flexWrap: 'wrap' }}>
                {anime.genres.map(g => (
                  <Link key={g} to={`/catalog?genre=${encodeURIComponent(g)}`} style={{ padding: '3px 10px', borderRadius: 8, border: '1px solid var(--border)', background: 'var(--bg-elevated)', color: 'var(--text-muted)', fontSize: 11, fontWeight: 500, textDecoration: 'none', minHeight: 'unset', minWidth: 'unset', transition: 'all 0.2s' }}>{g}</Link>
                ))}
              </div>
            )}
          </>
