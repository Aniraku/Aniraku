...
We run Aniraku at [www.aniraku.tech](https://www.aniraku.tech). Here's what happens with your data.
...
Comments are posted through TheAnimeCommunity's comment widget — see Third parties below.

## What we don't

- No tracking cookies, no ad networks on our own pages. We do load Vercel Web Analytics and Speed Insights (cookieless, aggregate page views and page-load performance only) and Google Analytics (aggregate usage under Google's policies, blockable through browser controls). No cross-site tracking, no advertising profiles.
- Your data isn't sold to anyone, shared with advertisers, or used to build profiles about you.
- We don't track you across other websites.

## Where stuff lives

- Account data, watch history, bookmarks, and settings are stored in Supabase (hosted on AWS, encrypted at rest). Every request is scoped to your login — neither the app nor our backend ever sees anyone else's data, and row-level security prevents one account from reading another.
...
- **Streaming backend and video hosts (Anikoto, FlixCloud and similar video hosts)** — video streams are resolved by our backend (`api.aniraku.tech`) and are played through its proxy where possible, but some connections are made directly from your browser to the video CDN — in those cases the CDN operator can see your IP address. These third-party hosts may display ads inside their players.
- **TheAnimeCommunity** — powers the comment widget on title and watch pages. The comments you post and your interactions with the widget are handled by their service under their own policy.
...
- **Vercel** — hosts the frontend and provides cookieless Web Analytics / Speed Insights.
- **Google Analytics** — aggregate page-view statistics for this site only.
- **Streaming backend host** — like any web server, it records basic request metadata (IP address, user agent) in transient logs for rate limiting and abuse prevention; these are not used for tracking.

## How long we keep things

- Account data? Until you delete your account (Settings → Danger Zone → Delete Account). Deleting wipes your profile, watch history, bookmarks, and settings — it cannot be undone.
