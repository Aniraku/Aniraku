...
The Preview catalog, search, title metadata, and mapping flow are **MAL-first** through the branch-local resolver. Only the visible Schedule and Next Airing data path uses the existing Aniraku API. Normal upcoming surfaces first request `GET /api/v1/schedule` and use the existing Aniraku GraphQL proxy if that endpoint has no usable rows; the calendar planner uses that proxy’s popular-releasing media feed and each title’s next confirmed airing entry. It matches production by showing Today first and the next six local dates, with no previously released episode rows and no direct AniList browser requests. Those returned records retain verified AniList IDs before existing routes and playback contracts consume them. The player supports adaptive playback, provider fallback, subtitles, dubs, quality selection, seeking, and intro/outro skipping where the data is available. Progress and personal context remain attached to the title instead of disappearing after playback.
