// TS port of Aniraku's src/pages/NewPassword.jsx — the recovery / password
// reset screen. Verbatim flow semantics: isSupabaseConfigured gate, recovery
// marker or PASSWORD_RECOVERY event → ready, URL token cleanup, one-shot
// updateUser + local signOut, redirect back to /login?password=updated.
...
const Wrapper = styled.main`
  min-height: 100vh;
  min-height: 100dvh; /* mobile pass: dynamic viewport — no URL-bar jump */
...
function passwordIssue(password: string, confirmPassword: string): string {
...
const NewPassword = () => {
  ...
  const [phase, setPhase] = useState<'verifying' | 'ready' | 'invalid' | 'complete'>('verifying');
  ...
  // Aniraku route title (Aniraku App.jsx routeTitles, brand-swapped).
  useEffect(() => {
    const previousTitle = document.title;
    document.title = 'Choose a New Password · Aniraku';
    return () => {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      const err = updateError as AuthErrorLike | undefined;
      setError(err?.message || 'We could not update your password. Request a new recovery link and try again.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <>
      <Wrapper id='main'>
        ...
        <Card aria-live='polite'>
          ...
          {phase === 'invalid' && (
            <Button type='button' $loading={false} onClick={() => navigate('/auth/forgot-password', { replace: true })}>Request a new link</Button>
          )}
        </Card>
        </Box>
      </Wrapper>
